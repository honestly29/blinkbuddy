...
def _make_profiler_result(stdout):
    """Create a mock subprocess result for system_profiler."""
    result = MagicMock()
...
def _make_swift_result(raw_value):
    """Create a mock subprocess result for the swift authorisation check."""
    result = MagicMock()
...
class TestCheckCameraPermission:
    ...
    def test_returns_unknown_when_status_is_authorised(self):
        ...
        be caused by something else (camera in use, some other error)."""
        with patch("python.camera_permission.platform.system", return_value="Darwin"), \
             patch("python.camera_permission.subprocess.run",
    ...
    def test_returns_unknown_when_swift_times_out(self):
        """If the swift command takes too long, fall back gracefully."""
        with patch("python.camera_permission.platform.system", return_value="Darwin"), \
             patch("python.camera_permission.subprocess.run",
    ...
    def test_returns_unknown_when_swift_not_found(self):
        """If swift CLI isn't available, fall back gracefully."""
        with patch("python.camera_permission.platform.system", return_value="Darwin"), \
             patch("python.camera_permission.subprocess.run",
                   side_effect=_dispatch_subprocess(
                       SYSTEM_PROFILER_WITH_CAMERA,
