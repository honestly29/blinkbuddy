...
- **Low-light detection bug.** In low-light conditions there is a known bug where the face-detection indicator can show green (i.e. the app has locked onto a face) while blink detection fails to register any blinks. The app does not currently detect this "face visible but blinks invisible" state and cannot warn the user. In good lighting conditions, detection works reliably.
